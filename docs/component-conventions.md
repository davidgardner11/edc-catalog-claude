...
What the Phase 3 card components actually do. Written after building them, from the code — not a style guide aspiration. Where a rule has an exception in the code, the exception is named.
...
- No `emits` anywhere yet, and no slots. Both were considered for card navigation and neither earned its keep — the card is a fixed composition, not a layout container.
...
Local `ref` per card, per ADR-004. `CardCarousel` owns `index`, `ColorwayGrid` owns `page`, and neither is lifted or persisted — a card that scrolls out of view and remounts starts over, which is intended. Nothing is stored in a composable yet; `useCatalogFilters` arrives in Phase 6.
...
- Mobile-first: the only responsive modifier used is `sm:` on two type sizes. The card itself does not respond to viewport at all — it is pinned to 260–320px everywhere, and the *page* grid (`repeat(auto-fill, minmax(260px, 320px))`) is what reflows.

## Accessibility

- **Every control is a real `<button type="button">` with an `aria-label`.** There are exactly three per card: two full-height carousel halves and the colorway pager. No click handlers on divs.
...
The card renders **no link to `/pack/[slug]`**, because that route does not exist yet and `nitro.prerender.crawlLinks` would try to prerender a 404. Instead `BackpackCard` and `CardLabel` take an optional `to`, and the model name becomes a `<NuxtLink>` the moment it is supplied. Phase 6 passes `` :to="`/pack/${pack.slug}`" `` and nothing else changes.
...
- `ColorwayGrid`'s pager still calls `stopPropagation()`, as ADR-015 requires, even though there is currently no ancestor handler for it to stop. It is a guard for the Phase 6 card-body handler, and the comment in the code says so — do not delete it as dead code.

## Testing hooks

No `data-testid` attributes were added. The Phase 7 E2E assertions (band ratios, 8 cells, wrap behaviour) are all reachable via role and accessible name, which is a stronger check because it fails if accessibility regresses. Add a testid only when a selector is genuinely unreachable that way, and note why.
