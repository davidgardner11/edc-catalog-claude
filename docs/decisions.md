...
- Runtime imports **between** modules in `app/utils/` are relative (`./format`), not `~/utils/format`. `~` is a Vite resolution, not a Node one, and the type-only imports that predate this were erased before anything had to resolve them. Relative keeps `app/utils/` loadable by a bare `vitest` run without a runner config — which is the arrangement Phase 7 inherits.
